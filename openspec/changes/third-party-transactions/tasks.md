...
- [x] 1.1 確認 Node.js >= 22、npm >= 10、Angular CLI 已安裝 (`node --version`, `ng version`)
- [x] 1.2 決定本機資料庫方案：本機 PostgreSQL / Docker PostgreSQL / Railway 雲端 DB
- [x] 1.3 複製 `.env.example` 為 `apps/api/.env`，填入 DATABASE_URL、API_SECRET_KEY、Discord Webhook URLs、ECPay/NewebPay 測試金鑰
- [x] 1.4 在 Discord 建立兩個 Webhook（#payment-success 和 #payment-alerts），取得 URL 填入 `.env`
...
- [x] 3.1 驗證 create transaction use case：`POST /api/transactions` 建立交易，確認 transaction entity enforces state machine 初始狀態為 `pending`，回傳包含 `paymentForm` HTML
- [x] 3.2 驗證 duplicate orderRef is rejected：相同 `orderRef` 發送兩次，第二次應回 `409 Conflict`（Clean Architecture 分層確保 DuplicateOrderRefError 從 domain 層正確傳遞至 presentation 層）
- [x] 3.3 確認 raw callback is always persisted：呼叫 `POST /api/callback/ecpay`，查 `payment_callbacks` 表確認有記錄
- [x] 3.4 驗證 handle payment callback use case（callback 先回 200 再非同步處理）：模擬 ECPay 成功回調（RtnCode=1），確認交易狀態從 `pending` 變 `success`，且 endpoint 立即回傳 `200 OK`
- [x] 3.5 模擬失敗回調（RtnCode=2），確認交易狀態從 `pending`/`processing` 變 `failed`
- [x] 3.6 驗證 invalid state transition is rejected：對已 `success` 的交易再次送成功回調，應回 `422 Unprocessable Entity`
- [x] 3.7 測試 list and detail query use cases：`GET /api/transactions`、`GET /api/transactions/:id` 回傳正確資料與分頁
- [x] 3.8 測試 get detail for non-existent transaction：未知 id 應回 `404 Not Found`

## 4. Discord 通知驗證 (Discord Notification Verification)

- [x] 4.1 先直接 `curl` Discord Webhook URL 確認 webhook 能用（送測試訊息到 #payment-success）
- [x] 4.2 觸發成功回調，確認 #payment-success 頻道收到綠色 embed（success notification sent to success webhook）
- [x] 4.3 觸發失敗回調，確認 #payment-alerts 頻道收到紅色 embed（failure notification sent to failure webhook）
- [x] 4.4 驗證 Discord errors do not block main flow：將 Webhook URL 改成無效 URL，確認 callback 端點仍回 `200 OK`，不受影響
- [x] 4.5 確認 notification is fire-and-forget（Discord 使用 Webhook 而非 Bot）：Discord 呼叫不 await，不阻塞 callback response；使用 `.catch()` 吸收錯誤

## 5. 付款閘道整合 (Payment Gateway Integration)

- [x] 5.1 確認 gateway factory resolves adapters by name：呼叫 `buildGatewayMap().get('ecpay')` 回傳 EcpayAdapter，`get('unknown')` 回傳 undefined
- [x] 5.2 使用 ECPay 測試帳號，實際送出付款表單到 ECPay staging 環境，確認 ECPay payment form generation 正常跳轉
- [x] 5.3 在 ECPay 測試環境完成付款，確認 ECPay callback verification（CheckMacValue 驗證通過）（本地環境 ReturnURL 無法被 ECPay staging 呼叫，已於 task 3.3–3.5 以手動模擬正確 CheckMacValue 驗證通過）
...
- [x] 6.1 在 `apps/payment-page` 執行 `ng new` 或補齊 `angular.json`、`main.ts`、`index.html`（參考 README）
- [x] 6.2 執行 `npm install` 安裝 Angular、PrimeNG、Tailwind CSS 依賴
...
- [x] 7.1 在 `apps/admin` 執行 `ng new` 或補齊設定檔（`--port 4201`）
- [x] 7.2 執行 `npm install`，執行 `ng serve --port 4201`，確認 `http://localhost:4201` 正常顯示
- [x] 7.3 確認 transaction list with pagination and filters 顯示交易列表，測試狀態與閘道篩選
- [x] 7.4 確認 transaction status is displayed with colored badge（success=綠，failed=紅）
- [x] 7.5 點擊詳情連結，確認 transaction detail page shows all fields 正確顯示所有欄位
- [x] 7.6 確認 API key is injected automatically in all requests（auth interceptor 已實作；API 回傳資料確認 X-Api-Key 正確送出）
- [x] 7.7 確認 signal-based state management：TransactionStateService 使用 signal() 管理狀態
